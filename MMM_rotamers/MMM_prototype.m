...
% Last Modified by GUIDE v2.5 13-Dec-2010 17:15:17
...
hMain=handles;
hMain.scriptpath=strcat(rootdir,'scripts/');
...
cmdpath=which('dssp.exe');
if isempty(cmdpath),
    add_msg_board('DSSP not implemented on this computer');
else
    input_file='p:/MMM/2JLN/2JLN_cut_471.pdb';
    output_file='p:/MMM/2JLN/2JLN.dssp';
    cmd=sprintf('%s %s %s %s',cmdpath,input_file,output_file);
    [status,result]=dos(cmd);
    if status,
        add_msg_board(sprintf('Error %i in DSSP call.',status));
    else
...
% --------------------------------------------------------------------
function menu_analysis_Callback(hObject, eventdata, ~)
...
seq1=model.structures{indices(1,1)}(indices(1,2)).sequence;
seq2=model.structures{indices(2,1)}(indices(2,2)).sequence;
adr1=mk_address(indices(1,1:2));
adr2=mk_address(indices(2,1:2));
...
if sum(nonmatch)>20,
    ...
    add_msg_board('Please use more sophisticated alignment tools, e.g. Clustal');
else
