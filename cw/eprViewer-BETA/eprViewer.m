function varargout = eprViewer(varargin)
...
% Last Modified by GUIDE v2.5 22-Mar-2013 13:41:14
...
if strcmp(files , '-- No file loaded --')
    ...
    files = sscanf(files,'%c',30);      % Cuts down to 25 characters
    files = sprintf('%-30s',files);     % Makes up to 25 characters
...
% If previous file then get string, add new string title to a new line
    ...
    next_formated = sscanf(next_line,'%c',30);
    next_formated = sprintf('%-30s',next_formated);
...
'#                                                                          ';...
'# This file has been created by eprViewer - v13.04 at:                     '];
...
title   = 'Export dataset to MATLAB Workspace - eprViewer v13.04';
lines   = 1;
default = {'x', 'y'};
